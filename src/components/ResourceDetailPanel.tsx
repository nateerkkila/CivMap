...
// This Comment interface remains the same, as it's the "correct" shape we want.
interface Comment {
  id: string;
  content: string;
  created_at: string;
...
export default function ResourceDetailPanel({ item, onClose }: ResourceDetailPanelProps) {
    ...
    if (item) {
      ...
      Promise.all([
        ...
        if (commentsRes.error) {
          ...
          // --- THIS IS THE FIX ---
          // The data from Supabase has `profiles` as an array, e.g., [{ username: 'nate' }]
          // We manually map over it to reshape it to what our `Comment` type expects.
          const formattedComments = commentsRes.data.map((comment: any) => ({
            ...comment,
            // If `profiles` is an array, take the first element. Otherwise, use it as is.
  ...
  const handleVote = async (direction: 1 | -1) => {
    ...
    if (error) {
      ...
      if (votesData) {
        setVotes({
          up: votesData.filter(v => v.direction === 1).length,
          down: votesData.filter(v => v.direction === -1).length,
        });
      }
  ...
  const handlePostComment = async () => {
    ...
    if (error) {
      ...
      // Manually reshape the newly created comment as well, just to be safe
    ...
    <div className="absolute top-4 left-4 z-[1000] w-[400px] max-w-[90vw] bg-white rounded-lg shadow-xl flex flex-col max-h-[calc(100vh-2rem)]">
      ...
      <div className="flex-1 p-6 overflow-y-auto">
        ...
        <div className="flex items-start justify-between text-sm text-gray-600 mb-4">
          ...
          <div className="flex items-center gap-4 text-gray-500 pt-2">
            <button onClick={() => handleVote(1)} disabled={!user} className={`flex items-center gap-1 cursor-pointer ${userVote?.direction === 1 ? 'text-green-600 font-bold' : 'hover:text-green-600'} disabled:cursor-not-allowed disabled:text-gray-400`}>
              <FaThumbsUp />
              <span>{votes.up}</span>
            </button>
            <button onClick={() => handleVote(-1)} disabled={!user} className={`flex items-center gap-1 cursor-pointer ${userVote?.direction === -1 ? 'text-red-600 font-bold' : 'hover:text-red-600'} disabled:cursor-not-allowed disabled:text-gray-400`}>
              <FaThumbsDown />
        ...
        {isProfileExpanded && profile && (
          <div className="mb-4 p-4 bg-gray-50 rounded-lg space-y-3 animate-fade-in">
            ...
            <div className="pt-2 border-t mt-3">
              <ProfileDetailRow icon={<FaPhone />} label="Phone" value={profile.phone} />
            </div>
          </div>
        )}
        ...
        {item.attributes?.image_url && (
          <div className="my-4">
            <img src={item.attributes.image_url as string} alt={item.general_description} className="w-full h-auto rounded-lg border" />
          </div>
        )}
            ...
            <div className="space-y-4">
              {comments.map(comment => (
                <div key={comment.id} className="flex items-start text-sm">
                  ...
                  <div>
                    <p>
                      <span className="font-semibold text-gray-800">{comment.profiles?.username || 'Anonymous'}</span>
                      <span className="text-gray-500 ml-2">{formatDistanceToNow(new Date(comment.created_at), { addSuffix: true })}</span>
                    </p>
      ...
      <div className="flex-shrink-0 p-4 border-t bg-gray-50 rounded-b-lg">
        <div className="flex items-center gap-3">
          <textarea
            placeholder={user ? "What do you think?" : "You must be logged in to comment."}
            rows={1}
            value={newComment}
            onChange={(e) => setNewComment(e.target.value)}
            disabled={!user}
            className="flex-1 block w-full px-3 py-2 text-sm text-gray-700 bg-white border border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 disabled:bg-gray-100"
          />
          <button 
            onClick={handlePostComment} 
            disabled={!user || newComment.trim() === ''}
            className="px-4 py-2 text-sm font-semibold text-white bg-gray-700 rounded-md hover:bg-gray-800 disabled:bg-gray-400 disabled:cursor-not-allowed"
          >
            SEND
          </button>
        </div>
